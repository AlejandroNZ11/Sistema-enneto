...
export class AgregarHallazgo3Component implements OnInit{
  ...
  dientesSuperiorInfante: string[] = ['55', '54', '53', '52', '51', '61', '62', '63', '64', '65'];
  dientesInferiorInfante: string[] = ['85', '84', '83', '82', '81', '71', '72', '73', '74', '75'];
  ...
  llenarSelect(): void {
    ...
    else if(this.dientesSuperiorInfante.includes(this.numeroDiente$)){

      const indice = this.dientesSuperiorInfante.findIndex(num => num=== this.numeroDiente$);
      if (indice !== -1) {
        this.numerosMostrar = this.dientesSuperiorInfante.slice(indice + 1);
      } else {
        this.numerosMostrar = [];
      }
    }
    else if(this.dientesInferiorInfante.includes(this.numeroDiente$)){

      const indice = this.dientesInferiorInfante.findIndex(num => num=== this.numeroDiente$);
      if (indice !== -1) {
        this.numerosMostrar = this.dientesInferiorInfante.slice(indice + 1);
      } else {
        this.numerosMostrar = [];
      }
    }
