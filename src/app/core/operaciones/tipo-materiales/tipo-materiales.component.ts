...
export class TipoMaterialesComponent implements OnInit{
  ...
  public ListTipomateriales: Array<Itipomateriales> = [];
  ...
  private getTableData(): void {
    ...
    this.tipomaterialesService.obtenerTipomateriales(env.clinicaId,this.currentPage, this.pageSize).subscribe((data: DataTipomateriales) => {
  ...
  public searchData(value: any): void {
    ...
    this.ListTipomateriales = this.dataSource.filteredData;
  }
  public sortData(sort: Sort) {
    const data = this.ListTipomateriales.slice();

    if (!sort.active || sort.direction === '') {
      this.ListTipomateriales = data;
    } else {
      this.ListTipomateriales = data.sort((a, b) => {
  ...
  editarTipomateriales(tipomateriales: Itipomateriales) {
    this.bsModalRef = this.modalService.show(EditarTipoMaterialesComponent);
    this.bsModalRef.content.tipomaterialesSeleccionada = tipomateriales.tipomaterialesId;
    this.bsModalRef.onHidden?.subscribe(() => {
      this.getTableData();
    });
  }
        ...
        this.tipomaterialesService.eliminarTipomateriales(tipomaterialesId).subscribe(
          (response) => {
            if (response.isSuccess) {
              Swal.fire('Correcto', 'Tipo material Eliminado en el sistema correctamente.', 'success');
              this.getTableData();
              return;
            } else {
