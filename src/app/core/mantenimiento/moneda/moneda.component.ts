...
import { DataMoneda, ITipoMoneda, Moneda } from 'src/app/shared/models/moneda';
...
export class MonedaComponent implements OnInit {
  ...
  ListMoneda: Array<ITipoMoneda> = [];
  columnas: string[] = []
  acciones: string[] = []
  monedaSeleccionada: Moneda = new Moneda();
  dataSource!: MatTableDataSource<ITipoMoneda>;
  ...
  private getTableData(currentPage: number, pageSize: number): void {
    ...
    this.monedaService.obtenerMonedas(env.clinicaId, currentPage, pageSize).subscribe((data: DataMoneda) => {
      ...
      this.dataSource = new MatTableDataSource<ITipoMoneda>(this.ListMoneda);
    });
  }
  ...
  editarMoneda(moneda: ITipoMoneda) {
    const initialState = {
